...
[[file:screenshots/anakondo-auto-completion-no-repl-demo.gif][Example completion using anakondo]]
...
+ Non exclusive completion, so if anakondo fails to find completions, Emacs will fallback to the other configured completions
